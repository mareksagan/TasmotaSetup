...
static const char* const YC01_PF_NAMES[] = {
  "Generic", "CherryTomato", "BeefsteakTomato", "BellPepper_Poblano_BananaPepper",
  "ChiliPepper_Jalapeno_Cayenne", "Habanero", "Eggplant", "Okra",
  "BushBeans", "PoleBeans", "Peas_SnowPeas_SugarSnapPeas", "Edamame",
  ...
  "Arugula", "Endive_Escarole_Frisee_Radicchio", "Mache", "Microgreens",
  "BokChoy_Senposai_YukinaSavoy", "Tatsoi_Komatsuna_Mibuna", "MustardGreens", "Mizuna",
  "Watercress", "Amaranth", "MalabarSpinach", "Purslane", "Sorrel", "Celtuce",
  "SweetPotatoVine", "GarlicChives", "Basil_ThaiBasil_HolyBasil", "Parsley", "Cilantro",
  "Dill", "Mint_Peppermint_Spearmint", "Chives", "Oregano", "Thyme_Sage_Tarragon_Marjoram",
  "Rosemary_BayLaurel", "LemonBalm", "Lemongrass", "Stevia", "Shiso",
  "VietnameseCoriander", "Culantro", "Epazote", "Lovage", "SummerSavory", "WinterSavory",
  "Lavender", "Chamomile", "Feverfew_Hyssop_Echinacea", "Fennel",
  "Nasturtium_Calendula_Borage_Pansy_Viola", "Petunia", "GerberaDaisy", "Zinnia",
  "Snapdragon", "Begonia_Impatiens", "SweetAlyssum", "Lobelia", "Marigold", "Dianthus",
  "Cornflower", "Portulaca", "Ginger_Turmeric", "Claytonia", "LandCress", "WelshOnion",
  "Daylily", "FavaBeans", "BearsGarlic", "Lettuce_Spinach_Kale",
  "Lettuce_Arugula_Chard", "Lettuce_Basil_Parsley", "Spinach_Kale_Chard", "Microgreens1",
  "SaladGreens", "Lettuce_Spinach_Arugula_Radish", "BokChoy_Tatsoi_Komatsuna1",
  "Mizuna_Mibuna_Senposai", "Basil_Mint_Parsley", "Cilantro_Dill_Chives",
  "Rosemary_Thyme_Oregano", "LemonBalm_Chamomile_Mint", "Mediterranean_Herb",
  "Tomato_Pepper_Eggplant", "CherryTomato_BellPepper", "Pepper_Cucumber",
  "Eggplant_Zucchini_Pepper", "Tomato_CherryTomato_Roma", "BellPepper_ChiliPepper_Jalapeno",
  "Radish_Turnip_Carrot", "GreenOnion_GarlicChives_Leek", "Onion_Shallot_Chive",
  "Petunia_Lobelia_Alyssum", "BushBeans_PoleBeans_Edamame",
  "Watercress_Purslane_Sorrel", "Radicchio_Mache1",
  "BeetGreens_TurnipGreens_CollardGreens", "Tomato_Basil",
  "Shiso_VietnameseCoriander_Culantro", "Lavender_Rosemary_Sage"
};

static const float YC01_PF_DATA[][17] = {
  {5.8,6.2,1000,2500,20,25,0.2,300,450,7500,0,0,0,0,0,0,0},
  {5.8,6.0,1400,2000,20,25,0.2,300,450,750,1,6.0,6.3,1800,2800,20,26},
  {5.8,6.0,1400,2000,20,25,0.2,300,450,750,1,6.0,6.3,2000,3500,20,26},
  {5.8,6.0,1200,1800,22,26,0.2,300,450,750,1,6.0,6.3,1600,2400,22,26},
  {5.8,6.0,1200,1900,22,26,0.2,300,450,750,1,6.0,6.3,1600,2600,22,27},
  {5.8,6.0,1200,2000,22,27,0.2,300,450,750,1,6.0,6.3,1600,2800,22,28},
  {5.8,6.0,1200,1800,22,26,0.2,300,450,750,1,6.0,6.3,1600,2400,22,26},
  {5.8,6.2,1200,1800,22,27,0.2,300,450,750,1,6.0,6.5,1400,2200,22,28},
  {5.8,6.2,1000,1400,18,24,0.2,300,450,500,1,6.0,6.3,1400,2000,18,24},
  {5.8,6.2,1000,1400,18,24,0.2,300,450,500,1,6.0,6.3,1400,2200,18,24},
  {5.8,6.2,800,1200,15,20,0.2,300,450,500,1,6.0,6.3,1200,1800,15,20},
  {5.8,6.2,1000,1400,18,24,0.2,300,450,750,1,6.0,6.3,1400,2000,18,24},
  {5.8,6.2,1200,1600,22,26,0.2,300,450,750,1,6.0,6.3,1600,2400,22,26},
  {5.5,5.8,1000,1400,15,22,0.1,300,450,500,1,6.0,6.2,1400,2000,15,22},
  {6.0,6.2,1200,2000,18,22,0.1,300,450,5000,0,0,0,0,0,0,0},
  {6.0,6.5,1200,1600,18,22,0.1,300,450,5000,0,0,0,0,0,0,0},
  {6.0,6.5,1200,1800,18,22,0.2,300,450,10000,0,0,0,0,0,0,0},
  {6.0,6.2,1500,1800,18,22,0.2,300,450,5000,0,0,0,0,0,0,0},
  {6.0,6.2,1000,1400,18,22,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,800,1200,15,20,0.2,300,450,7500,0,0,0,0,0,0,0},
  {5.5,5.8,400,700,18,22,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,1200,1600,18,22,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.2,1000,1400,18,22,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.2,1200,1600,18,22,0.2,300,450,10000,0,0,0,0,0,0,0},
  {6.0,6.2,1000,1400,18,22,0.2,300,450,10000,0,0,0,0,0,0,0},
  {6.0,6.5,700,1100,18,22,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,1200,1600,20,24,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,1200,1600,22,26,0.1,300,450,5000,0,0,0,0,0,0,0},
  {6.0,6.5,800,1200,18,22,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,1000,1400,18,22,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,1200,1600,18,22,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,1000,1400,18,22,0.2,300,450,5000,0,0,0,0,0,0,0},
  {5.8,6.2,1200,1800,20,24,0.1,300,450,5000,0,0,0,0,0,0,0},
  {6.0,6.5,1000,1600,18,22,0.2,300,450,5000,0,0,0,0,0,0,0},
  {6.0,6.5,1000,1400,18,22,0.2,300,450,5000,0,0,0,0,0,0,0},
  {5.8,6.2,1200,1600,18,22,0.2,300,450,5000,0,0,0,0,0,0,0},
  {6.0,6.5,1200,1800,18,22,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,1000,1600,18,22,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,1200,1800,18,24,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,1000,1600,18,22,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,1400,2000,20,24,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,1000,1600,20,24,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.8,1000,1400,18,22,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,800,1200,18,22,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,1000,1400,18,22,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.8,1000,1400,18,22,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,1200,1800,20,26,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,1400,2000,22,28,0.2,300,450,8000,0,0,0,0,0,0,0},
  {5.8,6.5,800,1200,10,20,0.2,300,450,7500,0,0,0,0,0,0,0},
  {5.8,6.5,700,1200,10,20,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,900,1300,10,22,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.5,800,1400,15,24,0.2,300,450,7500,0,0,0,0,0,0,0},
  {5.8,6.2,800,1200,10,20,0.2,300,450,500,1,6.0,6.3,1200,1800,10,20},
  {6.0,6.5,800,1200,8,18,0.2,300,450,5000,0,0,0,0,0,0,0},
  {5.8,6.2,1000,1600,18,22,0.1,300,450,5000,0,0,0,0,0,0,0},
  {5.8,6.2,1000,1600,18,22,0.1,300,450,5000,0,0,0,0,0,0,0},
  {5.8,6.2,900,1400,18,22,0.1,300,450,5000,0,0,0,0,0,0,0},
  {6.0,6.5,1200,1600,18,22,0.1,300,450,7500,0,0,0,0,0,0,0},
  {5.8,6.2,500,900,18,22,0.1,300,450,4000,0,0,0,0,0,0,0},
  {5.8,6.2,900,1400,18,22,0.1,300,450,5000,0,0,0,0,0,0,0},
  {5.8,6.2,900,1400,18,22,0.1,300,450,5000,0,0,0,0,0,0,0},
  {6.0,6.5,1000,1400,18,22,0.1,300,450,5000,0,0,0,0,0,0,0},
  {6.0,6.2,1000,1400,18,22,0.1,300,450,5000,0,0,0,0,0,0,0},
  {5.8,6.5,1200,1600,18,22,0.1,300,450,5000,0,0,0,0,0,0,0},
  {6.0,6.5,1200,1600,18,24,0.2,300,450,7500,0,0,0,0,0,0,0},
  {5.8,6.5,1000,1400,18,22,0.1,300,450,5000,0,0,0,0,0,0,0},
  {6.0,6.5,1200,1600,20,24,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.3,1600,2400,22,26,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.3,1600,2200,22,26,0.2,300,450,7500,0,0,0,0,0,0,0},
  {5.8,6.2,1400,2200,22,25,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.3,1600,2200,22,26,0.2,300,450,7500,0,0,0,0,0,0,0},
  {5.8,6.3,1400,2400,20,26,0.2,300,450,7500,0,0,0,0,0,0,0},
  {6.0,6.3,1600,2400,22,27,0.2,300,450,7500,0,0,0,0,0,0,0},
...
static bool yc01_nimble_init = false;
static NimBLEClient *yc01_client = nullptr;
...
static void yc01_apply_profile(const float *v) {
  ...
  for (int i = 0; i < 8; i++) {
    yc01_ranges.fmar[i] = (yc01_ranges.fmax[i] - yc01_ranges.fmin[i]) * 0.15f;
...
static void yc01_apply_boost(const float *v) {
  if (v[10] < 1.0f) return;
  yc01_ranges.fmin[0] = v[11];
  yc01_ranges.fmax[0] = v[12];
  yc01_ranges.fmin[1] = v[13];
  yc01_ranges.fmax[1] = v[14];
  yc01_ranges.fmin[2] = v[13] * 0.64f;
  yc01_ranges.fmax[2] = v[14] * 0.64f;
  yc01_ranges.fmin[5] = v[15];
  yc01_ranges.fmax[5] = v[16];
}

static const char* yc01_color(float v, int idx) {
  if (idx < 0 || idx > 7) return "green";
...
static int yc01_find_profile(const char *name) {
  for (size_t i = 0; i < YC01_PF_COUNT; i++) {
    if (strcmp(YC01_PF_NAMES[i], name) == 0) return (int)i;
  }
  return -1;
}
...
static void yc01_load_profile(const char *name) {
  ...
  if (yc01_boost && YC01_PF_DATA[idx][10] > 0) {
    yc01_apply_boost(YC01_PF_DATA[idx]);
  }
}

static void yc01_ble_cleanup(void);
static void yc01_ble_cleanup(void) {
  if (yc01_client) {
    if (yc01_client->isConnected()) yc01_client->disconnect();
    NimBLEDevice::deleteClient(yc01_client);
    yc01_client = nullptr;
  }
}
...
static bool yc01_ble_do_read(void) {
  if (!yc01_nimble_init) {
    ...
    yc01_nimble_init = true;
  }
  yc01_client = NimBLEDevice::createClient();
  if (!yc01_client) return false;
  yc01_client->setConnectionParams(12, 12, 0, 51);
  yc01_client->setConnectTimeout(15);
  uint8_t mac[6];
  yc01_parse_mac(yc01_mac, mac, nullptr);
  bool ok = yc01_client->connect(NimBLEAddress(mac, yc01_addr_type), true);
  if (!ok) {
    AddLog(LOG_LEVEL_DEBUG, PSTR("YC01: connect failed"));
    yc01_ble_cleanup();
    return false;
  }
  NimBLERemoteService *pSvc = yc01_client->getService(NimBLEUUID((uint16_t)YC01_SVC_UUID));
  if (!pSvc) {
  ...
  uint8_t buf[32];
  size_t len = value.length() > 32 ? 32 : value.length();
...
static void CmndYC01Profile(void) {
  if (strlen(XdrvMailbox.data) == 0) {
    Response_P(PSTR("{\"YC01\":{\"Profile\":\"%s\",\"Boost\":\"%s\"}}"),
               yc01_profile_name.c_str(), yc01_boost ? "on" : "off");
    return;
  }
...
void (*const YC01_Commands[])(void) PROGMEM = {
  &CmndYC01, &CmndYC01Read, &CmndYC01Poll, &CmndYC01Profile, &CmndYC01Boost, &CmndYC01Mac
};
...
static void YC01Init(void) {
  ...
  if (yc01_find_profile(yc01_profile_name.c_str()) < 0) {
    yc01_profile_name = "Generic";
  }
  yc01_load_profile(yc01_profile_name.c_str());
...
static void YC01EverySecond(void) {
  ...
  if (yc01_state == YC01_OK) {
    yc01_fails = 0;
    yc01_retry_cnt = 0;
    yc01_last_ok = millis() / 1000;
    yc01_state = YC01_IDLE;
  } else if (yc01_state == YC01_FAIL) {
    yc01_fails++;
    if (yc01_retry_cnt < 2) {
      yc01_retry_cnt++;
  ...
  xSemaphoreGive(yc01_mutex);
  if (can_poll) {
    if (yc01_retry_cnt > 0 && millis() / 1000 >= yc01_retry_at) {
      yc01_retry_at = millis() / 1000 + 3;
      yc01_start_op();
      return;
    }
