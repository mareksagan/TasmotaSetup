...
struct generic_sensor_t {
  ...
  uint32_t keepAliveMs; // keep GATT connection open after this operation succeeds
...
// Persistent-connection keep-alive state
static uint32_t BLEKeepAliveUntil = 0;
static NimBLEAddress BLEKeepAliveAddr;
...
static void BLETaskRunCurrentOperation(BLE_ESP32::generic_sensor_t** pCurrentOperation, NimBLEClient **ppClient){
  ...
  if ((*pCurrentOperation)->notifytimer){
    ...
    diff = diff/1000;
    if (diff > 20000){ // 20s
      // If we already have read data, a missing notify is not a failure.
      if ((*pCurrentOperation)->readlen > 0){
        AddLog(LOG_LEVEL_INFO,PSTR("BLE: notify timeout, but read data available - treating as success"));
        (*pCurrentOperation)->state = GEN_STATE_READDONE;
      } else {
#ifdef BLE_ESP32_DEBUG
  ...
  if (pClient->isConnected()){
    if (BLEKeepAliveAddr == op->addr){
#ifdef BLE_ESP32_DEBUG
      if (BLEDebugMode > 0) AddLog(LOG_LEVEL_DEBUG,PSTR("BLE: BLETask: reusing kept-alive connection"));
#endif
      reusedConnection = true;
    } else {
      AddLog(LOG_LEVEL_INFO,PSTR("BLE: disconnecting kept-alive connection for different device"));
      pClient->disconnect();
      BLEKeepAliveUntil = 0;
      return;
    }
  ...
  bool connectOk = reusedConnection || pClient->connect(op->addr, true);
  if (connectOk) {
...
static void BLETaskRunTaskDoneOperation(BLE_ESP32::generic_sensor_t** op, NimBLEClient **ppClient){
  bool success = (((*op)->state == GEN_STATE_READDONE) || ((*op)->state == GEN_STATE_WRITEDONE) || ((*op)->state == GEN_STATE_NOTIFIED));
  bool keepAlive = success && ((*op)->keepAliveMs > 0) && ((*ppClient)->isConnected());

  if (keepAlive){
    BLEKeepAliveUntil = millis() + (*op)->keepAliveMs;
    BLEKeepAliveAddr = (*op)->addr;
#ifdef BLE_ESP32_DEBUG
    if (BLEDebugMode > 0) AddLog(LOG_LEVEL_DEBUG,PSTR("BLE: runTaskDoneOperation: keeping connection alive for %u ms"), (*op)->keepAliveMs);
#endif
  } else {
    if ((*ppClient)->isConnected()){
#ifdef BLE_ESP32_DEBUG
...
static void BLEOperationTask(void *pvParameters){
  ...
  for(;;){
    ...
    // keep-alive maintenance: if a connection is being kept open but no
    // operation for that peer is queued, disconnect when the keep-alive expires.
    if (BLEKeepAliveUntil && pClient->isConnected()){
      bool stillNeeded = false;
      if (currentOperation && (BLEKeepAliveAddr == currentOperation->addr)) stillNeeded = true;
      if (!stillNeeded){
        TasAutoMutex localmutex(&BLEOperationsRecursiveMutex, "BLEKeepAlive");
        for (int i = 0; i < queuedOperations.size(); i++){
          if (queuedOperations[i] && (BLEKeepAliveAddr == queuedOperations[i]->addr)){
            stillNeeded = true;
            break;
          }
        }
      }
      if (!stillNeeded && (millis() > BLEKeepAliveUntil)){
        AddLog(LOG_LEVEL_INFO,PSTR("BLE: keep-alive expired, disconnecting"));
        pClient->disconnect();
        BLEKeepAliveUntil = 0;
      }
    }
    ...
    // While a connection is kept alive we don't scan, so the BLE advert
    // watchdog would otherwise restart the stack.  Feed the watchdog.
    if (BLEKeepAliveUntil && pClient->isConnected()){
      BLEScanLastAdvertismentAt = esp_timer_get_time();
    }

    // start a scan if possible (don't scan while a GATT connection is kept open)
    if (((BLEMode == BLEModeRegularScan) || (BLETriggerScan)) && (BLEKeepAliveUntil == 0)){
      ...
      while (p){
        switch(*p | 0x20){
          ...
          case 'u': // 'unique' context for this request
            ...
            break;
          case 'k': // keep connection alive for N milliseconds after success
            prepOperation->keepAliveMs = strtoul(p+2, nullptr, 10);
            break;
          case 'r':
